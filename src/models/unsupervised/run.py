...
def main(args):
    ...
    raw_data: list[mne.io.Raw] = list(load_raw_data().values())
...
def parse_args():
    ...
    parser.add_argument(
        "--gmm-pca", type=int, default=3, help="Number of PCA components for GMM"
    )

    # HMM parameters
    parser.add_argument(
        "--hmm-states", type=int, default=2, help="Number of hidden states for HMM"
    )
    parser.add_argument(
        "--hmm-pca", type=int, default=1, help="Number of PCA components for HMM"
    )
