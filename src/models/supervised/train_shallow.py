"""
Trening modelu Shallow ConvNet (Schirrmeister et al., 2017) na zbiorze BCI-IV 2b.
Uruchomienie: python train_shallow.py --data-dir ./data --epochs 150 --eval-mode per-subj
"""
...
# ──────────────────────────────────────────────────────────────────────────────
# 1. FILE DISCOVERY & DATA LOADING (Identyczne jak w EEGNet dla uczciwego porównania)
# ──────────────────────────────────────────────────────────────────────────────
...
def load_gdf(path: Path):
    ...
    # Przywrócona pełna logika z oryginalnego skryptu EEGNet
    MI_KEYS = {"769", "770"}
    MI_VALS = {1, 2, 769, 770}
...
# ──────────────────────────────────────────────────────────────────────────────
# 2. NOWY MODEL: SHALLOW CONVNET
# ──────────────────────────────────────────────────────────────────────────────
class Square(nn.Module):
    """Zastępuje standardową aktywację w celu wyliczenia mocy sygnału."""

    def forward(self, x): return torch.pow(x, 2)


class Log(nn.Module):
    """Logarytmuje wyjście (jak w analizie FBCSP). Dodano eps dla stabilności."""
...
class ShallowConvNet(nn.Module):
    """
    Shallow Convolutional Neural Network (Schirrmeister et al., 2017)
    Wejście: (Batch, 1, n_channels, n_times)
    """

    def __init__(self, n_classes=2, n_channels=3, n_times=500, n_filters_time=40, filter_time_length=25,
                 pool_time_length=75, pool_time_stride=15, dropout=0.5):
        super().__init__()

        # Ekstrakcja cech w czasie i przestrzeni
        self.temporal_spatial = nn.Sequential(
            # Konwolucja czasowa (Bandpass filter proxy)
            nn.Conv2d(1, n_filters_time, kernel_size=(1, filter_time_length), bias=False),
            # Konwolucja przestrzenna (CSP proxy)
            nn.Conv2d(n_filters_time, n_filters_time, kernel_size=(n_channels, 1), bias=False),
    ...
    def forward(self, x):
        features = self.temporal_spatial(x)
...
def main(args):
    ...
    for subj in all_subjects:
        ...
        # DODANO: Sprawdzanie czy brakuje pary plików
    ...
    # Zastąpienie EEGNet modelem ShallowConvNet
    model = ShallowConvNet(n_classes=2, n_channels=n_ch, n_times=n_times, dropout=args.dropout).to(device)
    ...
    print(f"\nTrening Shallow ConvNet przez {args.epochs} epok...\n")
    ...
    print(f"\n{'=' * 56}")
    print(f"WYNIKI SHALLOW CONVNET (Najlepsza epoka: {best_epoch})")
    print(f"{'=' * 56}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train Shallow ConvNet model on BCI-IV 2b dataset")
    parser.add_argument("--data-dir", default="./data", help="Path to data directory")
    parser.add_argument("--eval-mode", choices=["pooled", "per-subj"], default="per-subj", help="Evaluation mode")
    parser.add_argument("--epochs", type=int, default=150, help="Number of epochs")
    parser.add_argument("--lr", type=float, default=1e-3, help="Learning rate")
    parser.add_argument("--weight-decay", type=float, default=1e-4, help="Weight decay")
    parser.add_argument("--dropout", type=float, default=0.5, help="Dropout rate")
    parser.add_argument("--batch-size", type=int, default=32, help="Batch size")
    parser.add_argument("--save", default="shallow_best.pt", help="Path to save model")
