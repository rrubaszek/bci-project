"""
Trening modeli opartych na Geometrii Riemanna (MDM oraz TS+LR) na zbiorze BCI-IV 2b.
Uruchomienie: python train_riemann.py --data-dir ./data
"""
...
# Biblioteka do Geometrii Riemanna
...
def main(args):
    ...
    # Definicja naszych modeli Riemanna
    models = {
        "MDM": Pipeline([
            ('cov', Covariances(estimator='oas')),  # Estymacja macierzy kowariancji
            ('mdm', MDM(metric=dict(mean='riemann', distance='riemann')))  # Odległość riemannowska
        ]),
        "TangentSpace_LR": Pipeline([
            ('cov', Covariances(estimator='oas')),
            ('ts', TangentSpace(metric='riemann')),  # Rzutowanie na przestrzeń styczną
            ('clf', LogisticRegression(C=1.0))  # Klasyczna regresja logistyczna
        ])
    }
    ...
    for subj in all_subjects:
        ...
        print(f"\n[{subj}] Trenowanie modeli Riemanna dla pacjenta {subj}...")
        ...
        for model_name, pipeline in models.items():
            ...
            print(f"  -> {model_name:18s} Accuracy: {acc:.4f}")

    # =========================================================
    # PODSUMOWANIE I DRUKOWANIE WYNIKÓW
    # =========================================================
    print("\n" + "=" * 60)
    print("WYNIKI KOŃCOWE (Geometria Riemanna) - Per-Subject")
    print("=" * 60)

    header = f"{'Pacjent':^10} | {'MDM':^15} | {'TangentSpace_LR':^20}"
    print(header)
    print("-" * len(header))

    for subj in sorted(results["MDM"].keys()):
        row = f"{subj:^10} | "
        row += f"{results['MDM'][subj] * 100:^15.2f} | "
        row += f"{results['TangentSpace_LR'][subj] * 100:^20.2f}"
        print(row)

    print("-" * len(header))
    avg_row = f"{'ŚREDNIA':^10} | "
    avg_row += f"{np.mean(list(results['MDM'].values())) * 100:^15.2f} | "
    avg_row += f"{np.mean(list(results['TangentSpace_LR'].values())) * 100:^20.2f}"
    print(avg_row)
    print("=" * 60)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train Riemannian geometry models on BCI-IV 2b dataset")
